...
// Function to adjust ranking cards for mobile
...
function animateStats() {
    ...
    statNumbers.forEach((statNumber) => {
        const targetValue = parseInt(statNumber.getAttribute("data-count"), 10);
        const hasPlusSign = statNumber.textContent.includes('+');
        let currentValue = 0;
        const increment = Math.max(1, Math.ceil(targetValue / 100)); // Ensure at least 1
        
        // Animation interval
        const interval = setInterval(() => {
            currentValue += increment;
            
            // Stop the interval when we reach or exceed the target value
            ...
            statNumber.textContent = currentValue + (hasPlusSign ? '+' : '');
...
document.addEventListener('DOMContentLoaded', function() {
    // Adjust ranking cards for mobile
    adjustRankingForMobile();
    
    // Animate stats if they exist on the page
